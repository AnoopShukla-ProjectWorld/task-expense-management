...
import { 
  ...
  FaProjectDiagram, FaCheckCircle, FaHourglassHalf, FaFolderOpen
} from "react-icons/fa";
...
const TaskCreatePage = () => {
  ...
  // Fetch all users to list both employees and managers
  ...
  const watchCustomProject = watch("custom_project_name");
  const watchAssignee = watch("assigned_to");
  const watchStart = watch("start_date");
  const watchDue = watch("due_date");
  const watchPriority = watch("priority");

  // Determine Project Name for Gantt Preview
  let selectedProjectName = "General Task (No Project Bound)";
  if (watchProject === "CUSTOM") {
    selectedProjectName = watchCustomProject || "Other / Custom Scope";
  } else if (watchProject) {
    selectedProjectName = projects?.find(
      (p) => String(p.id) === String(watchProject)
    )?.project_name || "General Task (No Project Bound)";
  }

  // Determine Assignee Name for Gantt Preview
  const selectedAssignee = activeUsers.find(
    (u) => String(u.id) === String(watchAssignee)
  );
  const selectedAssigneeName = selectedAssignee 
    ? `${selectedAssignee.full_name} (${selectedAssignee.role?.toUpperCase()})`
    : "Unassigned Operator";
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // If "Other" is selected, append custom project name to description
      let finalDescription = data.description || "";
      if (data.project_id === "CUSTOM" && data.custom_project_name) {
        finalDescription = `[Context Scope: ${data.custom_project_name}]\n\n${finalDescription}`;
      }
      formData.append("description", finalDescription);
      
      // If project_id is empty or CUSTOM, we send it as null/empty in database (making it a general task)
      if (data.project_id && data.project_id !== "CUSTOM") {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text-primary)]">
            Deploy Task Blueprint (Admin)
          </h1>
          <p className="text-xs text-[var(--text-secondary)] mt-0.5">
            Create corporate tasks, bind optionally to active projects, and delegate directly to Managers or Employees.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-7 space-y-5 glass-panel p-6 sm:p-8 border border-[var(--border-color)] bg-[var(--bg-secondary)] shadow-sm">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">Project Binding (Optional)</label>
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setIsCustomProject(val === "CUSTOM");
                  if (val !== "CUSTOM") {
                    setValue("custom_project_name", "");
                  }
                }}
              ...
              >
                ...
                <option value="CUSTOM" className="text-blue-500 font-bold">✨ Other / Custom Scope...</option>
              </select>
            </div>
          </div>

          {/* Custom Project Context Input Box */}
          {isCustomProject && (
            <motion.div 
            ...
            >
              <label className="text-xs font-bold text-blue-600 uppercase tracking-wider">Specify Task Context / Category *</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-blue-400">
                  <FaFolderOpen className="text-sm" />
                </span>
                <input
                  type="text"
                  placeholder="e.g. Corporate Audit, IT Maintenance, HR Onboarding"
                  required
                  {...register("custom_project_name", { required: "Please specify custom context name" })}
                  className="w-full pl-11 pr-4 py-3 bg-blue-50/10 border border-blue-200 rounded-xl text-sm text-[var(--text-primary)] placeholder-[var(--text-secondary)]/50 focus:outline-none focus:border-blue-500/50 transition-all font-semibold"
                />
              </div>
            </motion.div>
          )}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                autoComplete="off"
                placeholder="Review and approve team Q2 expense ledger"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <textarea
                rows={3}
                placeholder="Outline core requirements, expected outcomes, and procedural logs..."
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Unified Assignee select showing Managers and Employees */}
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {managers.length > 0 && (
                    <optgroup label="Managers (Direct Reports)">
                      {managers.map((m) => (
                  ...
                  {employees.length > 0 && (
                    <optgroup label="Employees (Corporate Staff)">
                      {employees.map((emp) => (
        ...
        {/* Right Panel: Gantt Roadmap Flow */}
              ...
              <div className="space-y-6 relative py-2 pl-4">
                ...
                <div className="relative flex gap-3.5 items-start">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-[var(--text-secondary)] truncate max-w-[240px] font-semibold text-blue-600">
                      {selectedProjectName}
                    </p>
                  </div>
                </div>

                {/* Node 2: Associate Assignment */}
                <div className="relative flex gap-3.5 items-start">
                  ...
                  <div className="space-y-0.5">
                    <h5 className="text-xs font-bold text-[var(--text-primary)]">Assigned Associate</h5>
